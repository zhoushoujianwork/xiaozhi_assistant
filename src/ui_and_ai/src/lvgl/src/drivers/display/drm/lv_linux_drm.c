...
typedef struct {
    ...
    drm_buffer_t drm_bufs_rotate[2];
    bool rotate;
} drm_dev_t;
...
static int drm_setup(drm_dev_t * drm_dev, const char * device_path, uint32_t connect_id, unsigned int fourcc);
static int drm_allocate_dumb(drm_dev_t * drm_dev, drm_buffer_t * buf, bool rotate);
...
lv_display_t * lv_linux_drm_create(int width, int height, bool rotate)
{
    ...
    drm_dev->rotate = rotate;
    if(rotate){
        drm_dev->width = height;
        drm_dev->height = width;
        drm_dev->mmWidth = height;
        drm_dev->mmHeight = width;
    }else{
        drm_dev->width = width;
        drm_dev->height = height;
        drm_dev->mmWidth = width;
        drm_dev->mmHeight = height;
    }
    
    lv_display_t * disp = lv_display_create(drm_dev->width, drm_dev->height);
    if(disp == NULL) {
        lv_free(drm_dev);
        return NULL;
    }
    ...
    //LV_LOG_USER("[lv_linux_drm_create] drm_dev: %p", drm_dev);
...
void lv_linux_drm_init(lv_display_t * disp, int fd, uint32_t conn_id, uint32_t crtc_id, uint32_t plane_id, uint32_t blob_id)
{
    ...
    drm_dev->plane_id = plane_id;
    drm_dev->crtc_id = crtc_id;
    drm_dev->conn_id = conn_id;
    drm_dev->blob_id = blob_id;
    drm_dev->plane = drmModeGetPlane(drm_dev->fd, plane_id);
    if(!drm_dev->plane) {
        LV_LOG_ERROR("Cannot get plane");
    }

    drm_dev->crtc = drmModeGetCrtc(drm_dev->fd, crtc_id);
    if(!drm_dev->crtc) {
        LV_LOG_ERROR("Cannot get crtc");
    }
  
    drm_dev->conn = drmModeGetConnector(drm_dev->fd, conn_id);
    if(!drm_dev->conn) {
        LV_LOG_ERROR("Cannot get connector");
    }

    lv_memcpy(&drm_dev->mode, &drm_dev->conn->modes[0], sizeof(drmModeModeInfo));
    ...
    LV_LOG_USER("Resolution is set to %" LV_PRId32 "x%" LV_PRId32 " at %" LV_PRId32 "dpi",
...
void lv_linux_drm_set_file(lv_display_t * disp, const char * file)
{
    ...
    ret = drm_setup(drm_dev, file, -1, DRM_FOURCC);
    if(ret) {
        close(drm_dev->fd);
        drm_dev->fd = -1;
        return;
                              ...
                              void * user_data)
{
    ...
    LV_LOG_USER("[page_flip_handler] get drm_dev %p", drm_dev);
    if(drm_dev!= NULL&&drm_dev->req) {
...
static int drm_dmabuf_set_plane(drm_dev_t * drm_dev, drm_buffer_t * buf)
{
    ...
    if(drm_dev->rotate){
        drm_add_plane_property(drm_dev, "SRC_W", drm_dev->height << 16);
        drm_add_plane_property(drm_dev, "SRC_H", drm_dev->width << 16);
        drm_add_plane_property(drm_dev, "CRTC_W", drm_dev->height);
        drm_add_plane_property(drm_dev, "CRTC_H", drm_dev->width);
    }else{
...
static int drm_setup(drm_dev_t * drm_dev, const char * device_path, uint32_t connect_id, unsigned int fourcc)
{
    ...
    ret = drm_find_connector(drm_dev, connect_id);
    if(ret) {
        LV_LOG_ERROR("available drm devices not found");
        goto err;
    }
    LV_LOG_INFO("drm: Found connector_id: %d\n",drm_dev->conn_id);
...
static int drm_allocate_dumb(drm_dev_t * drm_dev, drm_buffer_t * buf, bool rotate)
{
    ...
    if(rotate){
        creq.width = drm_dev->height;
        creq.height = drm_dev->width;
    ...
    LV_LOG_ERROR("size %lu pitch %u offset %u", buf->size, buf->pitch, buf->offset);
    ...
    if(rotate){
        ...
        offsets[0] = 0;
        ret = drmModeAddFB2(drm_dev->fd, drm_dev->height, drm_dev->width, drm_dev->fourcc,
...
static int drm_setup_buffers(drm_dev_t * drm_dev)
{
    ...
    ret = drm_allocate_dumb(drm_dev, &drm_dev->drm_bufs[0], false);
    if(ret)
        return ret;

    ret = drm_allocate_dumb(drm_dev, &drm_dev->drm_bufs[1], false);
    if(ret)
        return ret;

    /*Allocate rotate DUMB buffers*/
    if(drm_dev->rotate){
        ret = drm_allocate_dumb(drm_dev, &drm_dev->drm_bufs_rotate[0], true);
        if(ret)
            return ret;

        ret = drm_allocate_dumb(drm_dev, &drm_dev->drm_bufs_rotate[1], true);
        if(ret)
            return ret;
    }
    return 0;
...
static void drm_flush(lv_display_t * disp, const lv_area_t * area, uint8_t * px_map)
{
    ...
    for(int idx = 0; idx < 2; idx++) {
        if(drm_dev->drm_bufs[idx].map == px_map) {
            /*Request buffer swap*/
            if(drm_dev->rotate){
                lv_draw_sw_rotate(drm_dev->drm_bufs[idx].map, drm_dev->drm_bufs_rotate[idx].map, 800, 480, 3200, 1920, LV_DISPLAY_ROTATION_270, LV_COLOR_FORMAT_ARGB8888);
                if(drm_dmabuf_set_plane(drm_dev, &drm_dev->drm_bufs_rotate[idx])) {
